...
struct OpenBook {
    ...
    /// Manifest ids created by `add_resource` this session. Only these are
    /// ever sweep candidates (see `sweep_orphans`); an entry is dropped when
    /// its resource is removed or swept. Kept across saves so repeated
    /// edit/save cycles within one session stay correct.
    added_resources: HashSet<ResourceId>,
    /// Docs modified via `write_chapter` this session: resource id → the zip
    /// paths its CURRENT bytes reference (re-extracted on every write, a
    /// single-document parse). Kept across saves, mirroring the overlay.
    modified_doc_refs: HashMap<ResourceId, HashSet<String>>,
    /// Reference sets of modified docs BEFORE their first edit this session,
    /// captured once by `write_chapter`. Identifies pre-existing resources
    /// that lost a reference; the sweep conservatively keeps those (see
    /// `sweep_orphans`), so today this map only feeds a future explicit
    /// full-sweep clean-up action.
    original_doc_refs: HashMap<ResourceId, HashSet<String>>,
}
...
impl Session {
    ...
    pub fn open_book(&mut self, path: impl AsRef<Path>) -> CoreResult<Book> {
        ...
        self.books.insert(
            ...
            OpenBook {
                ...
                modified_doc_refs: HashMap::new(),
                original_doc_refs: HashMap::new(),
            },
        );
        Ok(book)
    }
    ...
    ///
    /// Before serialization an incremental orphan sweep drops resources
    /// that were added this session but end up unreferenced (manifest entry,
    /// overlay bytes, and zip entry) so repeated add/remove cycles cannot
    /// grow the file. Pre-existing resources are never dropped, and content
    /// documents, nav, NCX, and the cover are never swept (see
    /// `sweep_orphans`). The sweep reads no unmodified documents (#62).
    ...
    pub fn write_chapter(
        ...
        // Incremental-sweep bookkeeping (#62): on the FIRST modification of
        // this doc, remember what its pre-edit bytes referenced; on every
        // modification, re-extract what the new bytes reference. Both are
        // single-document parses — the sweep never reads unmodified docs.
        if !open.original_doc_refs.contains_key(resource_id) {
            let original = open
                .read_entry(&path)
                .map(|old| refs::xhtml_refs(&old, &path))
                .unwrap_or_default();
            open.original_doc_refs
                .insert(resource_id.to_owned(), original);
        }
        open.modified_doc_refs
            .insert(resource_id.to_owned(), refs::xhtml_refs(&bytes, &path));
    ...
    pub fn add_resource(
        ...
        open.added_resources.insert(resource_id.clone());
    ...
    pub fn remove_chapter(&mut self, book_id: &str, spine_item_id: &str) -> CoreResult<Book> {
            ...
            .map(|r| r.path.clone())
        {
            ...
            if !still_in_spine && !still_in_nav && !is_nav_doc {
                ...
                open.added_resources.remove(&resource_id);
                open.modified_doc_refs.remove(&resource_id);
                open.original_doc_refs.remove(&resource_id);
                if open.container.is_some() {
...
/// Garbage-collect session-added resources that end up unreferenced (the
/// incremental orphan sweep, #62).
///
/// The sweep never reads unmodified documents — it works entirely from
/// session bookkeeping, keeping `save_book` O(change), not O(book). Only
/// resources created by `add_resource` this session are candidates: they
/// cannot be referenced by documents that predate them, and every session
/// modification is tracked in `modified_doc_refs`, so their orphanhood is
/// fully decidable without touching the rest of the book. A candidate
/// survives iff some modified document's current bytes reference its path —
/// directly (`img src`, `link href`, SVG `xlink:href`, …) or transitively
/// through the `url(...)` chains of *added* CSS resources (whose bytes sit
/// in the overlay and are cheap to parse).
///
/// Documented limitation (core-api.md): PRE-EXISTING resources are never
/// swept, even when an edit this session removed their last reference
/// (`original_doc_refs` − `modified_doc_refs` identifies such candidates).
/// Proving their orphanhood would require reading every unmodified content
/// document — exactly the O(book) scan this sweep exists to avoid — so they
/// are conservatively kept; a full sweep can be offered later as an explicit
/// clean-up action (e.g. via validate/CLI). This still preserves the #53
/// fix: repeated add/remove of images within a session cannot grow the file.
///
/// Never swept regardless: content documents (XHTML/HTML media types, which
/// includes everything in the spine), the nav document, the NCX, and the
/// cover image. Orphans lose their manifest entry, their overlay bytes, and
/// their zip entry (via the `removed` raw-copy exclusion). EPUB 2 books are
/// read-only and never swept.
fn sweep_orphans(open: &mut OpenBook) {
    if open.book.epub_version == EpubVersion::V2 || open.added_resources.is_empty() {
        return;
    }
...
fn session_referenced_paths(open: &mut OpenBook) -> HashSet<String> {
    ...
    // Follow CSS chains through ADDED stylesheets only (added css → added
    // font/image). Kept added CSS contributes its refs; iterate to a fixed
    // point so css → css chains among added resources resolve too.
    let added_css: Vec<String> = open
        .book
        .resources
        .iter()
        .filter(|r| open.added_resources.contains(&r.id) && r.media_type == "text/css")
        .map(|r| r.path.clone())
        .collect();
    let mut scanned: HashSet<String> = HashSet::new();
    loop {
        let mut grew = false;
        for path in &added_css {
            if referenced.contains(path) && scanned.insert(path.clone()) {
                if let Ok(bytes) = open.read_entry(path) {
                    referenced.extend(refs::css_refs(&String::from_utf8_lossy(&bytes), path));
                    grew = true;
                }
            }
        }
...
<p>Second paragraph.</p>
    ...
    #[test]
    fn orphan_sweep_follows_added_css_chains_transitively() {
        // added css referencing an added font: the chain keeps the font;
        // an added font referenced by nothing is swept.
        let mut session = Session::new();
        let book = session.create_book(sample_metadata());
        session
            .add_resource(
        ...
        session
            .add_resource(
                ...
                "orphan.otf",
                "font/otf",
                b"orphan-font-bytes".to_vec(),
            )
            .unwrap();
        // Lands at OEBPS/images/main.css; url("kept.otf") resolves to
        // OEBPS/images/kept.otf next to it.
        session
            .add_resource(
                &book.id,
                "main.css",
                "text/css",
                br#"@font-face { font-family: K; src: url("kept.otf"); }"#.to_vec(),
            )
            .unwrap();
        session
            .write_chapter(
                ...
                ChapterContent {
                    resource: "titlepage".to_owned(),
                    format: ContentFormat::Xhtml,
                    content: r#"<html xmlns="http://www.w3.org/1999/xhtml"><head><link rel="stylesheet" type="text/css" href="images/main.css"/></head><body><p>x</p></body></html>"#.to_owned(),
                    fallback_reason: None,
                },
            )
            .unwrap();

        let target = temp_path("sweep-added-css-dst.epub");
        let saved = session
            .save_book(&book.id, Some(target.to_string_lossy().into_owned()))
            .unwrap();

        // chapter → added css → added font chain survives; the orphan font
        // (added, never referenced) is gone.
        for kept in ["main", "kept"] {
            assert!(
        ...
        assert!(names.iter().any(|n| n == "OEBPS/images/main.css"));
        assert!(names.iter().any(|n| n == "OEBPS/images/kept.otf"));
        assert!(!names.iter().any(|n| n == "OEBPS/images/orphan.otf"));
        session.open_book(&target).unwrap();
    }

    #[test]
    fn orphan_sweep_keeps_preexisting_resources() {
        // The incremental sweep (#62) never drops pre-existing resources:
        // neither ones that were never referenced, nor ones whose last
        // reference is removed by an edit this session (conservative keep —
        // proving orphanhood would mean reading every unmodified document).
        let opf = r#"<?xml version="1.0" encoding="UTF-8"?>
<package xmlns="http://www.idpf.org/2007/opf" version="3.0" unique-identifier="pub-id">
  <metadata xmlns:dc="http://purl.org/dc/elements/1.1/">
    <dc:identifier id="pub-id">urn:uuid:preexisting-keep</dc:identifier>
    <dc:title>Preexisting</dc:title>
    <dc:language>en</dc:language>
  </metadata>
  <manifest>
    <item id="nav" href="nav.xhtml" media-type="application/xhtml+xml" properties="nav"/>
    <item id="ch1" href="ch1.xhtml" media-type="application/xhtml+xml"/>
    <item id="style" href="style.css" media-type="text/css"/>
    <item id="loose" href="images/loose.png" media-type="image/png"/>
  </manifest>
  <spine>
    <itemref idref="ch1"/>
  </spine>
...
<body><nav epub:type="toc"><ol><li><a href="ch1.xhtml">One</a></li></ol></nav></body></html>"#;
        let entries = vec![
            ...
            ("OEBPS/style.css", CSS),
            ("OEBPS/images/loose.png", "loose-image-bytes"),
        ];
        let source = write_epub("sweep-preexisting-src.epub", &entries);
        let mut session = Session::new();
        let book = session.open_book(&source).unwrap();

        // Edit ch1 so it no longer links style.css: the stylesheet lost its
        // only reference this session, but as a pre-existing resource it is
        // still kept.
        session
            .write_chapter(
                &book.id,
                "ch1",
                md_chapter("ch1", "# One\n\nNo stylesheet any more.\n"),
            )
            .unwrap();
        let target = temp_path("sweep-preexisting-dst.epub");
        let saved = session
            .save_book(&book.id, Some(target.to_string_lossy().into_owned()))
            .unwrap();

        for kept in ["style", "loose"] {
            assert!(
                saved.resources.iter().any(|r| r.id == kept),
                "expected pre-existing {kept} to be kept by the sweep"
            );
        }
        let names = zip_names(&target);
        assert!(names.iter().any(|n| n == "OEBPS/style.css"));
        assert!(names.iter().any(|n| n == "OEBPS/images/loose.png"));
        let reopened = session.open_book(&target).unwrap();
        assert!(reopened.resources.iter().any(|r| r.id == "style"));
        assert!(reopened.resources.iter().any(|r| r.id == "loose"));
    }

    #[test]
    fn orphan_sweep_never_sweeps_added_cover() {
        // An added, otherwise-unreferenced image survives once it is the
        // cover: protection applies to session-added candidates too.
        let mut session = Session::new();
        let book = session.create_book(sample_metadata());
        let book = session
        ...
        metadata.cover_resource = Some("cover".to_owned());
        session.update_metadata(&book.id, metadata).unwrap();

        let target = temp_path("sweep-added-cover.epub");
        let saved = session
            .save_book(&book.id, Some(target.to_string_lossy().into_owned()))
            .unwrap();
        assert!(saved.resources.iter().any(|r| r.id == "cover"));
        assert!(zip_names(&target)
            .iter()
